...
- 브랜드 레드 #F04A28: **채운 레드 버튼은 화면당 한 곳**. 활성 칩·라벨은 틴트(red-10 배경 + red-600 글자), 클러스터 마커는 레드 원. 카테고리 색점: 새우구이(구이) #F0885C / 생새우회(회) #14957B (마커 링·카드 색점만). 화면 라벨은 "새우구이"·"생새우회".
...
- 지도 위에는 검색 바·칩 행 두 층만 — **둘 다 떠 있는 pill**(불투명 블록으로 지도를 덮지 않는다, decisions 2026-09-04). 로고·카운터·이벤트·주 버튼은 지도 위에 두지 않는다(시트로).
...
- **확인 높이는 실기기 등가로**: 390×**702**(사파리)·390×**656**(네이버·구글 인앱)·320×480. DevTools의 390×844는 브라우저 UI를 시뮬레이션하지 않아 **거짓**이다 — 실기기 웹은 656~702px만 쓴다. 세로 예산은 docs/design.md 공통 블록 (decisions 2026-09-04).
