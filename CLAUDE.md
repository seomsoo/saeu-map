...
## 지금 단계 (Phase 7 끝 — 런칭 2026-09-26, docs/plans/phase7-launch.md "## 결과")
Phase 6(백엔드)·Phase 7(런칭 준비: 하드닝·약관/방침·분석·서치어드바이저·keepalive 알림·사진 리사이즈·Turnstile 예비 토큰)이 prod에 배포됐고 폰 머니패스도 끝났다(2026-09-24). 코드 쪽 남은 건 없고, 런칭 뒤 백로그는 roadmap 백로그·decisions(Smart Placement, 신고 IP 해시 정리, 보안 ①②, 핀 카드 요청 시 생성, D5b). 새 작업은 항상 플랜 파일 → 승인 → 짧은 브랜치 → PR. **Supabase를 아는 파일은 `lib/server/`뿐**(`import "server-only"`). 컴포넌트·훅은 여전히 `lib/data.ts`만 부르고, `lib/data.ts`는 서버에선 `lib/server`를 직접, 클라이언트에선 Server Action을 부른다. **브라우저에 supabase-js를 싣지 않는다**(decisions 2026-09-10).
목 JSON(`lib/mock/`)은 지웠다(커밋 4). 설정값 JSON(이벤트 카드·까주기 테스트)은 `lib/content/`. 쓰기 액션은 예상 실패를 값(`Result`)으로 돌려주고 `lib/data.ts`가 throw로 바꾼다 — 프로덕션의 Next는 액션의 오류 메시지를 지우기 때문이다. 로컬 개발은 **Docker Desktop → `pnpm db:start` → `pnpm dev`**(.env.local에 로컬 Supabase 값 — Docker가 꺼져 있으면 홈이 에러 화면이다). 사진은 폰에서 먼저 1200px webp로 줄여 보내고(`lib/image-shrink.ts`) 서버가 다시 만든다; 쓰기 화면이 열리면 Turnstile 토큰을 미리 받아 둔다(`warmWriteGate`).
...
- 지도 위: **모바일은 검색 바·칩 행 두 층**, **데스크탑은 칩 행 한 층**(검색은 패널 안 — 결과가 나오는 표면에 입력을 붙인다, decisions 2026-09-08). 어느 쪽이든 **떠 있는 pill**이고 불투명 블록으로 지도를 덮지 않는다(decisions 2026-09-04). 로고·카운터·이벤트·주 버튼은 지도 위에 두지 않는다.
- 아이콘 coolicons(Iconify ci). 없으면 근접 대체하고 docs/decisions.md의 "커스텀 에셋 필요" 목록에 적어라.
- 금지: 크림+세리프, 카드 색 띠, 이모지 헤더, 그라데이션 배경, 보라색 (예외: 지하철 노선색은 사실 표기라 5호선 보라를 16px 배지 안에서만 쓴다 — decisions 2026-09-03).
...
- 쓰기는 상태 변화까지: "다녀왔어요"은 낙관적 업데이트 + 실패 롤백 (목 함수에 delay 400ms, 10% 실패).
- URL: /place/[id], /gu/[name], /test. 공유 링크로 해당 핀이 열려야 한다.
- 반응형: 320~1023px 유동 모바일 레이아웃(기준 뷰포트 390, 확인은 320/390/430), 1024px부터 **지도 위에 떠 있는 좌측 패널 420px**(여백 16·라운드 20, decisions 2026-09-08) + 풀블리드 지도(**같은 컴포넌트, CSS로 자리만 바꾼다**). 상세·제보·내 활동은 패널 전환, 지도가 필요 없는 오버레이(로그인·리뷰 폼·사유·신고·탈퇴)만 중앙 모달 480px (decisions 2026-09-07).
- **확인 높이는 실기기 등가로**: 390×**702**(사파리)·390×**656**(네이버·구글 인앱)·320×480. DevTools의 390×844는 브라우저 UI를 시뮬레이션하지 않아 **거짓**이다 — 실기기 웹은 656~702px만 쓴다. 세로 예산은 docs/design.md 공통 블록 (decisions 2026-09-04). 데스크탑은 **1440×900 · 1280×800 · 1024×768**.
