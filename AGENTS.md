...
### 데이터 경계
- 컴포넌트에서 데이터 직접 접근(fetch, DB 클라이언트 호출) 금지 — 모든 데이터는 lib/data.ts 경유. Supabase 클라이언트는 `lib/server/`에서만 만든다(`server-only`). `lib/server`를 import할 수 있는 건 `lib/data.ts`와 `app/` 라우트 핸들러(auth 콜백·사진 서빙)뿐.
...
### 보안
- 시크릿 하드코딩, 서버 전용 키(service_role 등)의 클라이언트 노출 금지. NEXT_PUBLIC_ 접두사는 네이버 지도 Client ID·카카오 JS 키(공유용)·GA4 측정 ID·Turnstile site key·Sentry DSN만 허용 (decisions 2026-09-08·2026-09-10). Supabase 키는 publishable도 서버 전용(브라우저에 supabase-js가 없다).
