 ...
 *
 * `expectedHost`(요청의 Host)를 주면 토큰이 **우리 페이지에서** 만들어졌는지도 본다 — 서버 액션은 같은 출처로만 오므로
 * 위젯이 돈 페이지의 hostname과 요청의 Host는 같아야 한다. 위젯의 호스트명 목록(대시보드)이 1차 방어고 이건 그 설정이
 * 풀렸을 때의 2차다(보안 리뷰 2026-09-16 #12). 테스트 키는 hostname이 늘 `example.com`이라 건너뛴다 —
 * `result_with_testing_key`는 테스트 secret만 돌려주므로 실서비스에서 이 분기로 빠질 수 없다.
 */
import "server-only";
...
/** 비교용 — 포트를 떼고 한글 도메인은 퓨니코드로(URL 파서가 한다). 못 읽으면 null */
...
export async function verifyTurnstile(token: string, secret: string, remoteIp?: string, expectedHost?: string): Promise<boolean> {
  ...
  try {
    ...
    if (expectedHost === undefined || json.metadata?.result_with_testing_key === true) return true;
    const got = hostnameOf(json.hostname);
    if (got !== null && got === hostnameOf(expectedHost)) return true;
