...
export async function openWriteGate(
  ...
  // Host만 본다 — x-forwarded-host는 클라이언트가 실어 보낼 수 있다. 없으면(null) 검증이 실패한다
  if (!(await verifyTurnstile(turnstileToken, env.TURNSTILE_SECRET_KEY, h.get("host"), ip))) return { failure: "bot check failed" };
