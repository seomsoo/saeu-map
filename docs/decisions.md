...
## 2026-09-03 — 빈 사진 상태: 128 타일 행 → 56px 한 줄

- **결정**: 사진이 0장이면 176×128 스트립 자리를 **56px 한 줄**(bg-sunken 라운드 12, 카메라 아이콘 + "첫 사진을 올려주세요" + chevron, 행 전체가 탭 영역)로 줄이고, "네이버에서 사진 보기"는 그 **아래 별도 줄**로 뺀다. 사진 행 전체 높이 144 → 98.
- **이유**: ① 사용자 피드백 — 빈 상태가 "애매하다". 원인은 한 행 안에 액션이 둘(행 전체 = 올리기, 안쪽 링크 = 네이버)이라 오른쪽 chevron이 어느 쪽을 가리키는지 알 수 없던 것. 겹친 표적을 세로로 떼어 냈다. ② **무사진이 기본값이다** — 목 50곳 중 47곳(94%)이 사진 없음. 기본값 화면의 맨 위 128px가 정보 0인 회색 타일이었다. 구글맵·카카오맵도 무사진이면 큰 플레이스홀더 대신 컴팩트한 "사진 추가" 한 줄이다.
- **뒤집은 결정**: 같은 날 "빈 상태 ＋ 타일 폭만 112로 좁힌다"(320 문구 접힘 대응)와 **"사진 유무로 행 높이가 안 바뀐다"는 불변식**. 높이를 맞추려고 빈 자리를 유지하는 건 무사진이 예외일 때만 옳다 — 94%에서는 반대다.
- **얻은 것**: 무사진 가게는 요약(270px)에서 **주소 줄까지** 보인다(사진 행 98 + 상호 블록 90 + 헤더 44 = 232). 사진 있는 가게의 수용 기준(확인 캡션까지)은 그대로다.
- **바꾸지 않은 것**: 스트립의 176×128과 ＋ 타일·10장 안내 타일, "사진이 있으면 네이버 링크는 8번 리뷰 끝"이라는 규칙.

## 2026-09-03 — 빈 사진 상태 3차: ＋ 타일 하나 + 액션 라벨 카피

- **결정**: 사진 0장이면 **스트립의 ＋ 타일 하나만** 남기고(176×128 그대로) 네이버 링크를 그 오른쪽 형제로 둔다. 빈 상태 전용 레이아웃·카피를 만들지 않는다. 타일 안 문구는 "사진"→**"사진 추가"**(사진이 있을 때와 같은 타일이라 라벨도 하나).
- **이유**: 하루에 두 번 다시 만든 자리다. ① 112 타일 + 오른쪽 텍스트 열 + chevron = "빈 회색 썸네일이 달린 리스트 행"으로 읽혔다(사용자: "애매하다"). ② 56px 전폭 바 = 세로로 납작하고 가로로만 꽉 찬 비례가 어색했다(사용자: "세로로 짧고 가로로만 풀로 한 게 별로"). 둘 다 **빈 상태를 위한 새 문법을 발명**해서 생긴 문제다. 스트립은 이미 ＋ 타일이라는 추가 어휘를 갖고 있으므로 빈 상태는 그 어휘의 0장 케이스일 뿐이다 — 분기 하나가 통째로 사라진다.
- **카피**: 요청문("첫 사진을 올려주세요")을 버리고 액션 라벨로. 지도·커머스 앱의 실제 표기는 네이버·카카오 "사진 등록", 구글 지도·당근 "사진 추가"이고, 요청문은 UI 라벨이 아니라 온보딩·마케팅 톤이다. 빈 타일 자체가 "없음"을 말하므로 문장으로 다시 말할 필요가 없다.
- **뒤집은 결정**: 같은 날 "56px 한 줄 + 아래 네이버 링크"(무사진 94% 근거로 높이를 줄인 것)와 그 앞의 "빈 상태 ＋ 타일만 112 폭". **"사진 유무로 행 높이가 안 바뀐다"(144) 불변식은 되살아난다** — 요약에서 확인 캡션까지 보이는 수용 기준이 두 상태 모두 여유 13px로 같다(390×844 실측). 무사진에서 주소 줄까지 보이던 이점은 포기했다.
- **320 대응**: 타일 176 + 간격 12를 빼면 링크 열이 96px이라 두 줄이 된다. 폭을 줄이는 대신 `break-keep`으로 "네이버에서 / 사진 보기 ↗"처럼 단어 경계에서 끊는다(없으면 "네이버에서 사/진 보기"로 잘린다).
