...
## 결과 (2026-09-24)
**6~7초 → 약 1초** (사용자 폰 실측, prod 수정 제안 — 상세를 열고 잠깐 뒤 제출). 챌린지가 지연의 대부분이었다는 추정이 맞았다. PR #22(`c86eced`), vitest +5. 계획과 같음 — `waitUntil`은 이미 있어 서버 쪽 변경 0. 런칭 뒤: Smart Placement(엣지 → DB 근처), 사진 리사이즈는 같은 날 #21에 먼저 들어감(plan photo-shrink).
