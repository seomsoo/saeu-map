...
| Codex PR #9 | P1(빈 `SITE_URL=`로 앱 기동 실패) 반영 + 테스트 3건, P2(경계 넘김 리센터)는 실측 뒤 보류 — decisions 2026-09-07 Codex PR #9 |
...
**남은 것**: Phase 7(서치어드바이저·`/test`·도메인), Phase 6(ISR/R2 캐시, 첫 PR에 Lighthouse 점수 PR 코멘트 — decisions), 백로그(LCP 셸 개선 — 청크 분할·폰트 preload, 키보드 단축키·포커스 링, 다크 모드·태블릿 전용 배치 없음 유지).
