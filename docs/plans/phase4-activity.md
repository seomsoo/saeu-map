...
**완료.** PR [#8](https://github.com/seomsoo/saeu-map/pull/8), 브랜치 `feat/phase4-activity`.

| 항목 | 결과 |
| --- | --- |
| 갭 스윕 | **미구현 0건** / 부분 4건 · 모호 4건 (전부 같은 날 수정·확정) |
| 테스트 | 319 → **377개** (심판대 제거 전 384) |
| 보안 리뷰 | 중간 1 · 낮음 4 반영, 정보 2는 주석 |
| Codex PR #8 | P2 3건 전부 반영 + CLAUDE.md 규칙 승격 2건 |
| 확인 뷰포트 | 390×702 · 390×656 · 320×480 (실기기 등가) + workerd(:8787) + 프리뷰 URL |

**계획에서 바뀐 것**
- **심판대(신규 패널)를 사용자 화면에서 뺐다.** 만들어 눌러 본 뒤 참여가 안 나온다고 판단 — 판단 비용(가봐야 안다)·모수(주 신규 0~2곳)·운영자가 더 낫다는 세 근거. 칩 [새로 들어온 집]과 `ChipKey`의 `"new"`까지 제거했다. 사유 시트(`flagPlace`)는 상세 맨 아래 [정보 수정 제안]으로 옮겨 살렸다 — 오류 신고는 참여와 성격이 다르다. 되살릴 조건·형태는 roadmap 백로그. (decisions 2026-09-05)
- **리뷰는 핀당 1개**로 확정. 내 리뷰가 있으면 기여 블록의 [리뷰 남기기]가 사라지고 수정·삭제는 리뷰 행이 갖는다. 데이터 계층이 `already reviewed`로 막는다.
- **내 활동은 닫으면 찜 탭으로** 돌아온다. 리뷰 수정은 별점·후기만(사진 교체는 Phase 6).

**갭 스윕이 잡은 부분 4건** (전부 수정)
1. 내 활동 ✕가 히스토리 엔트리를 남겨, 로그아웃·탈퇴 뒤 뒤로가기가 빈 패널을 다시 열었다 — 시트가 닫기 콜백에 MouseEvent를 넘긴 탓(`onClick={fn}`). 인자를 끊었다.
2. [탈퇴하기]가 레드가 아니라 잉크색 — `className` 덧칠이 cva 기본 유틸에 밀렸다. `danger` variant 신설.
3. 찜 탭에 로딩·에러가 없었다 — 로드 상태를 세션 id 비교로 파생.
4. 리뷰 폼 실패 문구는 이미 문서와 일치(에이전트가 옛 버전을 읽은 오탐).

**남은 것**: roadmap 백로그 3건(신규 필터 되살리기 · 제보 핀 확인일 표시 · 리뷰 폼 키보드 실기기).
