...
| # | 할 일 | 누가 | 상태 |
|---|---|---|---|
| **0-0** | **런칭 차단 — prod 카카오 로그인이 안 된다(사용자 보고 2026-09-21).** 실측: 로그인 시트 → 액션 → Supabase → 카카오 로그인 페이지까지는 정상, 인가 요청이 `scope=account_email profile_image profile_nickname`. GoTrue `kakao.go`가 `account_email`을 기본 스코프에 고정하고(추가만 되고 빼기는 없다), `email_optional`은 이메일 없는 유저를 받아 줄 뿐 스코프는 그대로다. 카카오 앱엔 이메일 동의항목이 없으므로(runbook 2-2) **KOE205**(설정하지 않은 동의항목 요청, 카카오 문서) 추정 — 화면 문구는 사용자 확인 전. 고치는 길: **A** 개인 개발자 비즈 앱 전환(사업자등록번호 불필요 — 본인인증 + 카카오비즈니스 통합 약관, 콘솔 [앱] > [일반] > [비즈니스 정보] > [개인 개발자 비즈 앱], 카카오 문서 app-setting/app) → 이메일 동의항목을 **선택 동의**로 · **B** 카카오 OIDC를 서버에서 직접 받고 `signInWithIdToken`(스코프를 우리가 정한다, 구조 변경 반나절~하루). 고른 쪽은 decisions에 기록 | 사용자(A 콘솔) / 내가(B) | **열림** |
