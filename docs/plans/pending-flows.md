...
### 결과 선언 뒤에 뒤집은 것 (같은 브랜치, 사용자 지적)

- **메뉴 수정이 제보의 구이/회 2줄 폼이었다.** 크롤 가게는 메뉴가 **중앙값 3줄·최대 5줄**(50곳 중 64%가 3줄 이상)이고 화면은 그 줄을 다 그리는데 폼만 2줄이라 나머지가 사라지는 것처럼 읽혔다. → **지금 있는 줄을 다 보여주고 가격만** 고치는 `MenuEditFields`로 재설계(줄마다 [없어졌어요] + 맨 아래 [메뉴 추가] 한 줄). 제보 폼은 그대로 뒀다 — 새 가게는 대표 한 줄이 맞고 "한 줄이면 돼요"는 이탈을 막으려 낮춘 문턱이다.
- **승인 큐 → 즉시 반영 + 사후 확인.** 근거는 스펙 안에 있던 판례다(제보는 가게를 통째로 만드는데도 즉시 노출 + 24시간 내 사후 확인). 1인 운영에서 큐는 밀리고, 밀리면 아무도 두 번 고쳐주지 않는다. **전제인 되돌리기**를 같이 넣었다 — `PlaceEdit`(이전 값·actor·시각) + `getPlaceEdits()`, 탈퇴 시 actor 해제. `/admin`의 "수정 제안 큐" 탭은 **"수정 이력(되돌리기)"**이 됐다.
- **익명 허용은 유지**(spec 5 경계 그대로). 익명도 id가 남아 되돌리기·섀도 밴·속도 제한이 된다.
- **로그인 공급자는 카카오만**으로 재확인. 카카오 심볼은 공식 에셋 대기(커뮤니티 아이콘 셋으로 때우지 않는다 — 카카오 가이드·권한 심사 항목).
- 실기기에서 잡은 것 하나 더: **[없어졌어요]의 취소선이 안 그려졌다** — `truncate`(overflow:hidden)가 독립 서식 문맥을 만들어 바깥 `line-through`가 전파되지 않는다. 안쪽 span으로 옮겼다.
