...
- [ ] 6문항 2축 → 4유형: 콘텐츠 JSON + 채점·궁합 판정(순수 함수) + `lib/data.ts` 추천 3종
- [ ] 표지·문항 화면(`/test`, `/test/with/[type]`) + "결과 만드는 중"
- [ ] 결과 화면 + 유형별 가게 3곳 + 공유 두 갈래(결과 카드 · 궁합 초대)
- [ ] 궁합 결과(`/test/[type]/[other]`) + 취향 갈릴 때 구이·회 겸업 집 추천
- [ ] OG 24장 빌드 시 생성 + sitemap + 이벤트 배너 `href` 복구(2026-09-08에 비워 둔 것)
