...
## Phase 5.6 — 까주기 테스트 `/test` (spec 8 · design 화면 11)

Phase 7 항목을 앞으로 당겼다(2026-09-09). 목 데이터로 만들 수 있는 마지막 사용자 화면이고, 새우철 런칭 바이럴 장치라 시즌 안에 서야 한다. 포맷 비교와 국내 사례 조사는 decisions.md 2026-09-09, 플랜은 docs/plans/peel-test.md.

- [x] 6문항 2축 → 4유형: 콘텐츠 JSON + 채점·궁합 판정(순수 함수) + `lib/data.ts` 추천 3종
- [x] 표지·문항 화면(`/test`, `/test/with/[type]`) + "결과 만드는 중"
- [x] 결과 화면 + 유형별 가게 3곳 + 공유 두 갈래(결과 카드 · 궁합 초대)
- [x] 궁합 결과(`/test/[type]/[other]`) + 취향 갈릴 때 구이·회 겸업 집 추천
- [x] OG 25장 빌드 시 생성 + sitemap + 이벤트 배너 `href` 복구(2026-09-08에 비워 둔 것)
- 완료: `/test` 네 라우트가 workerd에서 돌고 슬러그 밖은 진짜 404, 배너 → 테스트 → 결과 → 가게 상세가 한 바퀴, 갭 스윕 0건
...
- ~~**제보 핀의 확인일 표시가 거짓이다**~~ → **Phase 6에서 닫는다**(2026-09-10). 확인일이 checkins에서 계산되면 확인 0회 핀은 값이 비고, 카드·상세가 "○일 전 **등록**"으로 말한다.
- **시즌 스탬프 → 레벨·배지** — 조건: 런칭 후 참여 데이터(다녀왔어요 재방문율)가 보일 때. 먼저 내 활동에 "이번 새우철 다녀온 집 N곳" 한 줄(확인 기록이 재료라 백엔드 추가 0, spec 8). **레벨을 제보·다녀왔어요에 거는 안**(2026-09-10 제안)은 보류 — 확인 신호가 곧 상품인데 위치 검증이 없어 레벨 동기가 신호를 오염시키고, 참여 대부분이 익명이며, 1인 운영에 곡선·배지·부정 감지가 따라온다(decisions 2026-09-10).
...
- **핀 공유 카드를 요청 시 생성(2026-09-18 Paid 결정으로 가능해짐)** — 지금은 빌드 때 있던 가게만 카드가 있고 그 뒤 제보된 핀은 루트 카드로 간다(`placeOgImagePath`·`BUILD_AT`). Paid는 CPU 30초라 satori+resvg를 요청마다 돌리고 R2 캐시에 두면 된다. 조건: **Phase 7, 새 제보가 공유되기 시작할 때**.
...
- [x] ~~보안 하드닝 ②③④ + 리뷰 백로그 + 소스맵 배선~~(PR #18 머지·db push 2026-09-23) · ~~`/privacy`·`/terms` + 링크 자리 + keepalive 알림~~(PR #19 머지·발화 2026-09-23, D1·D1b·D1c·D7) · ~~런칭 전 추가 3건~~(폰 머니패스에서 발견: #21 사진 1MB + 폰 리사이즈, #22 Turnstile 예비 토큰 6~7초 → 1초, #23 흰 타일 — 2026-09-24)
