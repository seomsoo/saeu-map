 ...
 * 여기서 6px을 뺀 값을 예산으로 쓴다 — `estWidth`가 실측보다 **작게** 나오기 때문이다(아래).
 */
const LINE_BUDGET_PX = 259;

/** 배지 하나가 먹는 폭: 20px 원 + gap 1.5(6px). */
const BADGE_PX = 26;
 ...
 * 예산에서 6px을 미리 빼 둔다 — 이 여유가 없으면 265에 아슬아슬하게 걸린 라벨이 통과했다가 잘린다
 * (`총신대입구(이수)역 5번출구에서 320m` 추정 265 / 실측 265.8).
 */
 ...
 * 조합(동대문역사문화공원역 + 배지 3)만 해당하고 현재 목 50곳은 전부 출구를 유지한다.
