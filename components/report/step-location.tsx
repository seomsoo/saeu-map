...
export function StepLocation({
  ...
  const confirm = async () => {
    ...
    try {
      ...
      if (duplicate && !isDuplicateDismissed(duplicate.id)) {
        setCandidate(duplicate);
        onShowCandidate(duplicate);
        return;
      }
      onConfirm(duplicate?.id ?? null);
    } finally {
      setChecking(false);
    }
  };
        ...
        footer={
          <div className="flex gap-2">
            ...
            <Button
              ...
              className="flex-1"
              onClick={() => {
                onDismissDuplicate(candidate.id);
      ...
      >
        <div className="flex items-center gap-3 rounded-12 border border-line-hairline p-4">
          <div className="min-w-0 flex-1">
            <p className="truncate text-body-l-semibold text-fg">{candidate.name}</p>
            <p className="truncate text-caption-l-regular text-fg-tertiary">
              {[candidate.gu, ...candidate.tags.map((t) => TAG_LABELS[t])].join(" · ")}
            </p>
          </div>
          <span className="shrink-0 text-body-m-medium text-fg tabular-nums">
            {formatDistance(haversineKm(pin, candidate))}
          </span>
        </div>
      </StepFrame>
    );
